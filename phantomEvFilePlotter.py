...
def createUnitDictionary():
    ...
    distanceSr=("distance","$R_{\odot}$",D_sr)
    massG=("mass","g",M_g)
    massKg=("mass","kg",M_kg)
    massSm=("mass","$M_{\odot}$",M_sm)
    velocityCm_s=("velocity","$cms^{-1}$",D_cm/T_s)
    velocityM_s=("velocity","$ms^{-1}$",D_m/T_s)
    velocityKm_s=("velocity","$kms^{-1}$",(D_m/1000.0)/T_s)
    forceDyn=("force","dyn",(M_g*D_cm)/(T_s*T_s))
    forceN=("force","N",(M_kg*D_m)/(T_s*T_s))
    angularmomentumGcm2_s=("angular momentum","$gcm^{2}s^{-1}$",(M_g*D_cm*D_cm)/T_s)
    angularmomentumKgM2_s=("angular momentum","$kgm^{2}s^{-1}$",(M_kg*D_m*D_m)/T_s)
    torqueDynCm=("torque","dyn cm",(M_g*D_cm*D_cm)/(T_s*T_s))
    torqueNm=("torque","Nm",(M_kg*D_m*D_m)/(T_s*T_s))
    densityG_cm3=("density","$gcm^{-3}$",M_g/(D_cm*D_cm*D_cm))
    densityKg_m3=("density","$kgm^{-3}$",M_kg/(D_m*D_m*D_m))
